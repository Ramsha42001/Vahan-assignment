...
2. (optional) Navigate to  `client/services/apiConfig.js ` :
    Add baseURL   `http://localhost:8000` if not present to connect your server to frontend locally

3. (optional) Navigate to  `client/src/views/Home.jsx`:
    Add baseWebsocketURL to `ws://127.0.0.0.0:8000/api/chat?session_id=${currentSession.id}`  if not already present to connect websocket connection for chat
...
* `GET /api/metrics/sessions/{session_id}`
  * Description: Get metrics for specific chat session
  * Authentication: Required (JWT token)
  * Parameters:
    * `session_id`: Target session identifier
  * Returns: Session-specific analytics
